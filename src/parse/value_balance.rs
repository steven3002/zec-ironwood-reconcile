...
mod tests {
    ...
    fn a_migration_pair_nets_to_the_amount_moved() {
        // A turnstile migration spends from Orchard and creates into Ironwood. Orchard's
        // valueBalance is positive (value leaving), Ironwood's is negative (value
        // entering), so the deltas have opposite signs and equal magnitude.
